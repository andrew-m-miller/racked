...
- `docs/roadmap/*.md` — phase-by-phase design docs (see `docs/roadmap/README.md` for the index). Covers both shipped phases (check here for the rationale behind existing behavior before assuming something is dead weight) and planned ones written but not yet built.
...
- **Picking up roadmap work:** phases 1–5 + the AI-coach stretch are shipped; the next to build is **Phase 6 (test harness)**, then 7→10 in order (`docs/roadmap/README.md` has the dependency chart). Read the phase doc first, then implement under the conventions above — especially the storage-layer and schema-migration rules, which are the "implement it well" guardrails. When a phase ships, move its row from Planned to Shipped in the roadmap README and update anything here it makes stale (e.g. Phase 6 removes "no test suite exists" above).
