...
**Persistence — Supabase Postgres, seven tables, all RLS-scoped to `auth.uid() = user_id` (except `video_links`):**
...
- `docs/roadmap/*.md` — phase-by-phase design docs (see `docs/roadmap/README.md` for the index). All ten phases + the AI-coach stretch are shipped; check the relevant doc for the rationale behind existing behavior — including where implementation deliberately diverged from the plan — before assuming something is dead weight.
