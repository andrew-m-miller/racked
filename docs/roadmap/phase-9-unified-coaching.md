...
- **Shipped as a client-side auto-run instead:** a true Supabase cron would
  have to rebuild the recap in Deno, duplicating `recap.js` / `progression.js`
  / `planUtils.js` (which imports the bundled `exercises.json`) into the edge
  function — two sources of truth for the exact numbers this phase is
  consolidating. The opt-in auto-run reviews the *finished* week on the first
  open of a new one and caches it in `coach_runs`; same ready-on-Monday UX, no
  duplicated logic. A server cron stays open as a follow-on if the pure core
  ever becomes shareable with Deno.
